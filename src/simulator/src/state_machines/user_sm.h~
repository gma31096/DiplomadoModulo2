...
/*switch ( state ) {
        ...
        case 3:
                ...
                break;
        case 4:
                *movements = generate_output(FORWARD,0.5*Mag_Advance,max_twist);
                *next_state = 5;
                break;
        case 5:
                *movements = generate_output(RIGHT,Mag_Advance,pi*0.5);
	...
	case 7:
                *movements=generate_output(STOP,Mag_Advance,max_twist);
		if(derIsBlock(observations,size,0.055))	*next_state = 6;
		else *next_state = 108;
		break;
	case 108:
                *movements=generate_output(FORWARD,Mag_Advance*0.5,max_twist);
		*next_state = 8;
		break;
	case 8 :	
                *movements=generate_output(RIGHT,Mag_Advance,pi*0.5);
		*next_state = 6;
        	break;
	case 9 :
                *movements=generate_output(FORWARD,Mag_Advance*0.8,max_twist);
		*next_state = 0;
		break;
	case 10: 
                *movements=generate_output(RIGHT,Mag_Advance,pi*0.5);
	...
	case 11:
                *movements=generate_output(FORWARD,Mag_Advance*0.5,max_twist);
		if(frontIsClear(observations,size,0.035)) *next_state = 8;
		else *next_state = 2;
	default:
                *movements=generate_output(STOP,Mag_Advance,max_twist);
                *next_state = 0;
